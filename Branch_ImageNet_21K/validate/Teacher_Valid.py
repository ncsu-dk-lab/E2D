...
def get_args():
    ...
    parser.add_argument('--arch-path', type=str, default=None,
                        help='path to the architecture file')
...
def main():
    ...
    #print(f"Training classes ({len(train_dataset.classes)}): {train_dataset.classes[:10]} ...")
    #print(f"Training class_to_idx: {list(train_dataset.class_to_idx.items())[:10]} ...")
    ...
    #print(f"Validation classes ({len(val_dataset.classes)}): {val_dataset.classes[:10]} ...")
    #print(f"Validation class_to_idx: {list(val_dataset.class_to_idx.items())[:10]} ...")
    ...
    model = torchvision.models.__dict__[args.model](pretrained=False, num_classes=10450)
    model = nn.DataParallel(model).cuda()
    model.train()

    print("=> loading teacher model '{}'".format(args.arch_path))
    # load teacher model
    import timm

    model_teacher = timm.create_model(args.teacher_model, pretrained=False, num_classes=10450)
    ...
    args.teacher_model = model_teacher
    '''images, _ = next(iter(val_loader))
    images = images.cuda()
    with torch.no_grad():
        logits = model_teacher(images)
        print("Top-5 logits for validation sample:", logits[0].topk(5))

    images, _ = next(iter(train_loader))
    images = images.cuda()
    with torch.no_grad():
        logits = model_teacher(images)
        print("Top-5 logits for training sample:", logits[0].topk(5))
    #print(f"Teacher model classes ({len(teacher_classes)}): {teacher_classes[:10]} ...")
    #exit(0)'''
    ...
    print(f"Teacher model path : {args.arch_path}")
    original_val_acc = validate(model_teacher, args)
    print(f"Teacher accuracy on original classes: {original_val_acc}%")
    exit(0)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % 80 == 0 or epoch == args.epochs - 1:
...
def train(model, args, epoch=None):
    ...
    for batch_idx, (data, target) in enumerate(args.train_loader):
        ...
        '''if batch_idx == 0:  # Only print for the first batch to avoid flooding the log
            print("Teacher logits stats: mean={:.4f}, std={:.4f}, max={:.4f}, min={:.4f}".format(
                soft_label.mean().item(), soft_label.std().item(), soft_label.max().item(), soft_label.min().item()))
            teacher_probs = F.softmax(soft_label / args.temperature, dim=1)
            print("Teacher probs stats: mean={:.4f}, std={:.4f}, max={:.4f}, min={:.4f}".format(
        teacher_probs.mean().item(), teacher_probs.std().item(), teacher_probs.max().item(), teacher_probs.min().item()))
        '''
